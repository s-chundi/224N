...
class Hybrid(nn.Module):
    ...
    def __init__(self, word_vectors, char_vectors, hidden_size, drop_prob=0.):
        ...
        self.enc = layers.EmbedEncoderBlock(conv_num=4, hid_size=hidden_size, kernel=7)
    ...
    def forward(self, cw_idxs, qw_idxs, cc_idxs, qc_idxs):
        ...
        c_enc = torch.transpose(c_enc, 1, 2)
        q_enc = torch.transpose(q_enc, 1, 2)
...
class QANetBulky(nn.Module):
    def __init__(self, word_vectors, char_vectors, hid_size):
        super().__init__()
        # TODO remove hardcoded constants, and get the embed sizes from above
        self.emb = layers.Embedding(word_vectors, char_vectors, hid_size=hid_size)
        self.encoder = layers.EmbedEncoderBlock(conv_num=4, hid_size=hid_size, kernel=7, num_heads=4)
        self.biatt = layers.BiDAFAttention(hid_size)
        # TODO: possibly remove this conv and make a huge set of encoder blocks
        self.conv = nn.Conv1d(hid_size*4, hid_size, kernel_size=1)
        # To mimic original QANet
        self.stack_size = 10
        # Should I make these 3 separate stacks?
        self.stack = nn.ModuleList([layers.EmbedEncoderBlock(conv_num=2, hid_size=hid_size, kernel=5, num_heads=4) for _ in range(self.stack_size)])
        self.out = layers.Output(hid_size=hid_size)
